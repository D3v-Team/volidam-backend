...
import { seedLidStatuses } from './lid_statuses/task-status.seed';
...
async function bootstrap(): Promise<void> {
  ...
  const lidStatusModel = app.get<typeof LidStatus>(getModelToken(LidStatus));
  const lidStatusRoleModel = app.get<typeof LidStatusRole>(
    getModelToken(LidStatusRole),
  );
  await seedLidStatuses(lidStatusModel, lidStatusRoleModel);
  ...
  if (!isProd) {
    const swaggerConfig = new DocumentBuilder()
      ...
      .setVersion('1.0.0')
      .addServer('https://api.volidam.usdsoft.uz', 'Production')
  ...
  appLogger.log(`Volidam [${nodeEnv}] started on http://localhost:${port}/api`);
}
